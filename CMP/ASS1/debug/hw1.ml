...
(*this lets us skip white spaces*)
let add_skip nt = 
...
let nt_expr =
   let rec make () =
    ...
    let op = add_skip op in
    let nums = plus (delayed make) in
    let nums = add_skip nums in
  ...
  | ScmOp (op,xs) -> match xs with
                    ...
                    | x :: xs ->
                       ...
                       let s = s :: List.fold_right
                               (fun a b -> (List.hd (comp_simple2 reg op a)) :: b) 
                               xs
                               []
                       in s
;;
...
let check_valid ls =
  ...
  if not (List.mem (Reg 0) rg_ls) then
    ...
    let is_init = function
      ...
      | InstRR(Mov,r1,r2) -> Hashtbl.add hash r1 1; Hashtbl.mem hash r2
      | InstRR(_,r1,r2) -> Hashtbl.mem hash r1 && Hashtbl.mem hash r2 
    in
    andmap is_init ls    
;;


let get_scm_op_inst = function 
    | InstRI(Add,_,_) | InstRR(Add,_,_) ->ScmAdd
    | InstRI(Mul,_,_) | InstRR(Mul,_,_) ->ScmMul
    | InstRI(Sub,_,_) | InstRR(Sub,_,_) ->ScmSub
    | InstRI(Div,_,_) | InstRR(Div,_,_) ->ScmDiv
...
let get_num_ri =(fun x -> match x with 
            | InstRI (_,_, Imm x) -> ScmInt' x
            | _ ->raise X_This_should_not_happen
            ) 
let combine x y=  
  let x_n = get_num_ri x in
  let y_n = get_num_ri y in
  let ls = [x_n;y_n] in
  let op = get_scm_op_inst y in
  ScmOp'(op,ls) 
;; 

let is_simple_expr_set = function
  | [] -> true 
  | _ :: [] -> true
  | x :: xs -> andmap 
          (fun a -> match a with 
            | InstRI (Mov,_,_) -> false
            | InstRI (_,_,_) -> true
            | InstRR (_,_,_) -> true
           ) 
          xs
...
(* let us first consider the case where our insts list has only 1 mov *)
let rec non_trivial_case = function  
  | x::y::xs -> 
      let init = combine x y in
      List.fold_left 
      (fun b a -> 
        let a_op = get_scm_op_inst a in
        let num = get_num_ri a in
        let b = [b] @ [num] in
        ScmOp'(a_op,b)
       )
      init   
  ...
  | _ -> raise X_This_should_not_happen (*only lists > 1 here *)
;;
let decomp_valid  = function
  |InstRI(_ , _, Imm x) :: []  -> ScmInt' x (*This is the only valid case*)
  | _ as l -> non_trivial_case l
;;

let non_simple = 
  let rec mov_route acc beg = function
    | []  -> acc
    | InstRR(op,_,_) :: [] -> 
        let op = get_scm_op op in
        ScmOp'(op, [beg;acc])
    | InstRI(op,_,Imm x) :: InstRR(op2,_,_) :: [] ->
        let op = get_scm_op op in
        let op2 = get_scm_op op2 in
        ScmOp'(op2,[acc;ScmOp'(op,[beg;ScmInt' x])])  
    | InstRI(op,_,Imm w) :: InstRR(op2,_,_) :: x :: rst ->
        let op = get_scm_op op in
        let op2 = get_scm_op op2 in
        let acc = ScmOp'(op2,[acc;ScmOp'(op,[beg;(ScmInt' w)])])
        in
        (match x with 
          | InstRI(Mov,_,Imm z) -> 
              mov_route acc (ScmInt' z) rst  
          | InstRI(op3,_,Imm z) -> (*need to fix acc here *)
              op_route (ScmOp'((get_scm_op op3), [acc ;(ScmInt' z)]))
                         rst
          | _ -> raise (err "In inner match mov_route"))

    | InstRI(op,_,Imm w) :: tl->
        let op = get_scm_op op in
        let acc = ScmOp'(op,[acc;(ScmInt' w)]) in
        mov_route acc beg tl 
        
    | _ -> raise (err "In outer match mov_route") 
    
    and op_route acc = function 
    | [] -> acc
    | InstRI(op,_,Imm x) :: [] -> 
        let op = get_scm_op op in
        ScmOp'(op, [acc; ScmInt' x])
    | InstRI(Mov,_,Imm z) :: xs -> 
        mov_route acc (ScmInt' z) xs 
    | InstRI(op,_,Imm z) :: xs -> 
        let op = get_scm_op op in
        op_route (ScmOp'(op,[acc;ScmInt' z])) xs
    | _ -> raise (err "In op_route") 
  in
  function 
    | [] | [_] -> raise X_This_should_not_happen 
    | InstRI(_,_,Imm z) as x :: 
        ((InstRI(_,_,Imm w)) as y :: tl)  ->
    (match y with

      |InstRR (_,_,_) -> raise (err "InstRR in non_simple") 
      |InstRI (Mov,_,_) -> mov_route (ScmInt' w) (ScmInt' z) tl 
      |InstRI(op,_,_) -> op_route (combine x y) tl
      )
    | _ -> raise (err "In the beginning")
;; 
...
let decompile_assembly_program insts = match check_valid insts with
  | false -> Uninitialized
  |  _ -> match is_simple_expr_set insts with 
      | true -> decomp_valid insts
      | false -> non_simple insts
;;
...
let voom2 s =
  let s= string_to_list s in
  let (e,s) = Parsers.nt_expr s in
  compile_arith e 
;; 
